...
**Version:** 2.0  
**Date:** 23/06/2025  
**Status:** Approved — aligned with PMHDV thesis overview; implementation baseline complete (see §17)

**Supersedes:** `OEX_DetailedDesign_V1.md` (retained for history)
...
18. [Appendices](#appendix-a-glossary)
...
| `docs/OEX_DetailedDesign_V1.md` | Prior design version (superseded) |
| `docs/api/openapi.yaml` | REST API contract (OpenAPI 3) |
| `docs/DEPLOY.md` | Production deployment guide |
| `PrjMng/DECISIONS.md` | Locked scope and technology choices |
...
### 2.4. Alignment with PMHDV Thesis Overview

This design implements the **minimal v1 path** from the PMHDV system overview (Online MCQ Exam System):

| Thesis topic | OEX decision |
|--------------|--------------|
| Web + REST + relational DB | ✓ Implemented |
| Three roles (Admin, Teacher, Student) | ✓ No extra “class manager” role |
| Single correct answer MCQ | ✓ |
| Direct exam-to-student assignment | ✓ No class/course module |
| Timer, auto-submit, auto-grade | ✓ |
| Scale ~100–500 users | ✓ Assumption in §2.3 |

See [Appendix C](#appendix-c-mapping-to-pmhdv-thesis-overview) for full mapping and explicit exclusions.
...
|-------|------------|-------|
| **Frontend** | Vue.js 3 + Vite + TypeScript | SPA; Composition API (**not React** — D-02) |
| **UI styling** | Custom CSS design tokens | `frontend/src/styles/main.css` |
...
| **Dev environment** | Docker Compose | PostgreSQL for local dev |
| **API docs** | OpenAPI 3 | `docs/api/openapi.yaml` (manual) |
| **Testing** | Vitest + Supertest | 76 tests; API E2E smoke |
...
Student → "My Exams" → Start Attempt
    ...
    → Student answers (auto-save on option change, debounced ~500ms; also save before submit)
...
| POST | `/auth/change-password` | User | Change own password (**planned — not implemented**) |
| GET | `/auth/me` | User | Current user profile |

> **Note:** No public `/auth/register`. Admin creates users via `POST /users` (AUTH-06).
...
### 9.7.1. Teacher Student Lookup (Assignment UI)

| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/students` | List active students (`?search=`) for exam assignment (Teacher) |
...
| `/login` | Login | Public |
| `/` | Dashboard | All |
| `/admin/users` | User Management | Admin (**placeholder UI — GAP-01**) |
...
Question picker: searchable list from subject's question bank with checkbox + reorder (up/down buttons in v1; drag-and-drop optional enhancement).
...
Row action: "View Detail" (**UI planned — results table implemented; per-attempt detail view pending**)
...
| E2E | Login → take exam → submit → view result | **API E2E:** `tests/e2e-student-flow.test.ts` (Supertest). Browser E2E (Playwright) optional |
...
| Vietnamese UI | English only (thesis report may be Vietnamese; product UI is English) |
| Public self-registration | Admin-only user creation |
| Export results (Excel/PDF) | Teacher results table in UI only |
| Results statistics / charts | Not in v1 |
| File storage for question images | No FS tier in architecture |
...
```
OEX/
├── PrjMng/                            # STATUS, TASKS, CHANGELOG, DECISIONS
├── docs/
│   ├── basic_requirement.md
│   ├── OEX_DetailedDesign_V1.md       # superseded
│   ├── OEX_DetailedDesign_V2.md       # this document
│   ├── DEPLOY.md
│   └── api/
│       └── openapi.yaml
├── frontend/                          # Vue.js SPA
...
*Snapshot: 23/06/2025 — after Phase 7 integration.*

### 17.1. Completed (matches design)

| Area | Status |
|------|--------|
| Prisma schema (§8) | ✓ All core tables |
| Seed catalog | ✓ Admin, teachers, students, subjects, questions, exams, assignments, attempts |
| Auth login + JWT + role guards | ✓ |
| Admin user CRUD API | ✓ |
| Teacher: subjects, questions, exams, assign, results API + UI | ✓ |
| Student: my-exams, take exam, result API + UI | ✓ |
| Auto-grade, timer expiry, answer leak prevention | ✓ |
| OpenAPI + deploy guide | ✓ `docs/api/openapi.yaml`, `docs/DEPLOY.md` |
| Backend tests | ✓ 76 passing (`npm run test`) |
| Frontend build | ✓ `npm run build` |

### 17.2. Gaps (design vs code — to complete project)

| ID | Item | Design ref | Priority |
|----|------|------------|----------|
| GAP-01 | Admin User Management **UI** (placeholder) | §10.1 `/admin/users`, USR-* | **Must** for full admin UX |
| GAP-02 | Change password API + UI | AUTH-05, §9.3 | Should |
| GAP-03 | Teacher **attempt detail** (answers vs correct per student) | RES-05, §10.2.9 | **Must** for teacher results |
| GAP-04 | Browser E2E (Playwright) optional | §14.1 | Nice-to-have |
| GAP-05 | Thesis deliverable `.docx` (PMHDV format) | Thesis §12 | **Must** for graduation submission |
| GAP-06 | Manual test checklist / demo script | §14.1 | Should for defense |

### 17.3. Intentionally excluded (v1)

See [Section 15](#15-out-of-scope-v1). No implementation expected unless scope changes via `PrjMng/DECISIONS.md`.
...
| 2.0 | 23/06/2025 | — | PMHDV overview alignment; actual stack (Vue, custom CSS); OpenAPI/deploy refs; §17 implementation status; Appendix C mapping |

## Appendix C: Mapping to PMHDV Thesis Overview

Cross-reference with the PMHDV “Hệ thống thi trắc nghiệm trực tuyến” system summary.

### C.1. System summary

| Thesis item | OEX V2 |
|-------------|--------|
| Name / goal | OEX — same purpose (§1.2) |
| Scope: Web + REST + RDBMS | ✓ |
| Scale ~100–500 users | ✓ §2.3 |

### C.2. Technology

| Thesis proposal | OEX (locked) |
|-----------------|--------------|
| React + Tailwind | **Vue 3** + custom CSS (D-02) |
| Node + Express | ✓ (D-03) |
| PostgreSQL + Prisma | ✓ (D-04) |
| JWT + bcrypt | ✓ (D-12) |
| Docker Compose + VPS | ✓ §13, `DEPLOY.md` |

### C.3. Architecture

| Thesis diagram | OEX |
|----------------|-----|
| 3-tier Client–Server | ✓ §4 |
| File Storage (images) | **Omitted** — question images out of scope |

### C.4. Roles

| Thesis | OEX |
|--------|-----|
| Admin / Teacher / Student | ✓ §5 |
| Trưởng khoa / quản lý lớp | **Not included** |
| Admin manages subjects | **No** — subjects owned by teachers |

### C.5. Functional modules

| Module (thesis §5) | OEX status |
|--------------------|------------|
| 5.1 Auth — login | ✓ |
| 5.1 Register | **Admin creates users only** |
| 5.1 Change password | **Gap GAP-02** |
| 5.1 Admin CRUD users | API ✓, UI **GAP-01** |
| 5.2 Question bank | ✓ (2–6 options, not fixed 4) |
| 5.2 Multi-answer / images / Excel | **Out of scope §15** |
| 5.3 Exams — manual pick, timing, max attempts | ✓ |
| 5.3 Random N / shuffle | **Out of scope** |
| 5.3 Assign class or students | **Direct students only** (D-06) |
| 5.4 Online exam — timer, palette, autosave | ✓ (debounced save, not fixed 30s) |
| 5.4 Anti-cheat tab detection | **Out of scope** (D-09) |
| 5.5 Auto-grade + results | ✓ |
| 5.5 Export Excel/PDF | **Out of scope** |
| 5.6 Class module | **Out of scope** — simplified path chosen |

### C.6. Database

Thesis ERD includes `classes`, `shuffle_*` columns, optional images. **OEX core 9 tables** match implemented Prisma schema (§8); optional thesis tables not modeled.

### C.7. API

| Thesis endpoint | OEX endpoint |
|-----------------|--------------|
| `/auth/login` | ✓ |
| `/auth/register` | **Use `POST /users` (Admin)** |
| `/questions/import` | **Not implemented** |
| `/exams/{id}/assign` | `POST /exams/{id}/assignments` |
| `/exams/{id}/questions` (POST) | `PUT /exams/{id}/questions` |
| Attempts CRUD | ✓ §9.8 |
| `/exams/{id}/results` | ✓ |
| — | `GET /students` (teacher assign helper) |
| — | `GET /my-exams` |

### C.8. UI screens

| Thesis # | OEX route | Implemented |
|----------|-----------|-------------|
| 1 Login | `/login` | ✓ |
| 1 Register | — | N/A (admin-only) |
| 2 Dashboard | `/` | ✓ |
| 3–7 Teacher flows | `/subjects`, `/exams`, … | ✓ |
| 8 My exams | `/my-exams` | ✓ |
| 9 Exam taking | `/take/:attemptId` | ✓ |
| 10 Results | `/results/:attemptId` | ✓ |
| 11 Statistics/charts | — | Not in v1 |
| 12 Admin users | `/admin/users` | **Placeholder (GAP-01)** |

### C.9. UI language

Thesis asks whether Vietnamese UI is needed. **OEX product UI is English** (D-07). Thesis *document* may be Vietnamese.

### C.10. Decisions locked (thesis §13 questions)

| Question | Answer |
|----------|--------|
| Project name | **OEX** (generic online exam; not PMHDV-domain-specific) |
| Stack | Vue + Node + PostgreSQL |
| Minimal scope | Single answer; no Excel; no anti-cheat; direct assign |
| Class module | **No** |
| Design format | Markdown in `docs/` |
| UI language | **English** |
