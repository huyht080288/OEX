...
```
┌─────────────────────────────────────────────────────────────┐
│  PRESENTATION — Vue 3 SPA (Browser, port 5173)              │
└──────────────────────────┬──────────────────────────────────┘
                           │ HTTP/JSON REST + Bearer JWT
┌──────────────────────────▼──────────────────────────────────┐
│  APPLICATION — Node.js Express (port 3000, /api/v1)         │
│  Routes → Controllers → Services → Prisma                     │
└──────────────────────────┬──────────────────────────────────┘
                           │ SQL
┌──────────────────────────▼──────────────────────────────────┐
│  DATA — PostgreSQL 15 (port 5432, database oex)             │
└─────────────────────────────────────────────────────────────┘
```
...
flowchart TB
    ...
    subgraph State
        Pinia[stores/auth.ts]
        API[api/client.ts + modules]
    end
    App --> Layout
    Layout --> Nav
    Guard --> Views
    Views --> Pinia
    Views --> API
    API -->|HTTPS JSON| Backend[Express API]
```
...
flowchart LR
    ...
    P1[1.0 Auth Service]
    P2[2.0 User Service]
    P3[3.0 Subject/Question Service]
    P4[4.0 Exam Service]
    P5[5.0 Attempt/Grade Service]
    D1[(users)]
    D2[(subjects, questions, options)]
    D3[(exams, assignments, exam_questions)]
    D4[(attempts, attempt_answers)]

    Client -->|JWT requests| P1 & P2 & P3 & P4 & P5
    P1 --> D1
    P2 --> D1
    P3 --> D2
    P4 --> D3
...
flowchart TB
    User([Người dùng])
    P1[1.0 Login Flow]
    P2[2.0 Teacher CRUD Flow]
    P3[3.0 Exam Taking Flow]
    P4[4.0 Result Display]
    DS1[(localStorage token)]
    DS2[(Pinia auth state)]
    DS3[(Component local state answers)]
    API([REST API])
...
sequenceDiagram
    ...
    S->>API: POST /attempts/start {assignmentId}
    API->>DB: Validate + INSERT attempt
    API-->>S: questions (no isCorrect)
    loop Each answer change
        S->>API: PUT /attempts/:id/answers
        API->>DB: UPDATE attempt_answers
    end
    S->>API: POST /attempts/:id/submit
    API->>DB: Transaction grade + SUBMITTED
    API-->>S: score, review?
```
...
erDiagram
    ...
    users ||--o{ exam_assignments : student
    ...
    exam_assignments ||--o{ exam_attempts : has
    exam_attempts ||--o{ attempt_answers : has
